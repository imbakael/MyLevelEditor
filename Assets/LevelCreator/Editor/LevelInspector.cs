...
public class LevelInspector : Editor
{
    ...
    private Dictionary<PaletteItem.Category, string[]> GetAllTiles(Level level) {
        var result = new Dictionary<PaletteItem.Category, string[]>();
        if (level.CategoryPieces == null || level.CategoryPieces.Count == 0) {
            foreach (PaletteItem.Category item in level.Categories) {
                result.Add(item, new string[level.TotalColumns * level.TotalColumns]);
            }
            return result;
        }
        foreach (KeyValuePair<PaletteItem.Category, SpriteRenderer[]> item in level.CategoryPieces) {
            SpriteRenderer[] spriteRenderers = item.Value;
            result.Add(item.Key, GetSpriteNames(spriteRenderers, level));
        }
        return result;
    }
